...
source("U:/ORP Operations/Fisheries Program/E-Reporting/4.0 Pilot projects/Roving Monitor Pilot/Data/code/importRegions.R")
# ------------------------- #
...
xTable = tableHTML(xTotals, rownames = FALSE, widths = rep(140,8),
                   caption = "Summary of trips for Nov. 2018 by landing day.",
...
# 15 to 21
# ------------------------- #
x = nov %>% filter(GearType %in% c("POUND NET - FISH", "POTS - EEL", "TROTLINES", 
                                      "CRAB POTS", "SCRAPES/DREDGES")) %>%
...
x3 = nov %>% filter(fishery %in% "FinFish",
                    GearType %in% c("POUND NET - FISH", "POTS - EEL", "GILL NET - DRIFT", 
                                       "HOOK AND LINE", "COLLAPSIBLE TRAPS")) %>%
  dplyr::select(TripID, hr) %>%
  mutate(block = "Not in any block", 
...
xTable = htmlTable(xTotals, rnames = FALSE,
                   caption="Percent of trips for Nov. 2018 by time block",
                   header =  c("Time block",
