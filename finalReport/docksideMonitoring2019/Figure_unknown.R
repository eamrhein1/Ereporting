...
ggplot()+
  geom_line(data = boo[boo$x %in% 0.1,], aes(x = x, y = y), col = "gold", size = 3) +
  geom_line(data = boo[boo$x %in% 0.3,], aes(x = x, y = y), col = "red", size = 3) +
  geom_line(data = boo[boo$x %in% 0.5,], aes(x = x, y = y), col = "blue", size = 3) +
  #geom_ribbon(aes(ymin = 7, ymax = 13), fill = "red")+
  geom_point(data = x, aes(y=hr, x = mo, size = n, col = Fishery, shape = Fishery))+
  geom_point(data = xx, aes(y=hr, x = mo, size = n, col = Fishery, shape = Fishery))+
  scale_color_manual(values = c("black","white"))+
